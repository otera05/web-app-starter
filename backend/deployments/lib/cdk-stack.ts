...
export class CdkStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    } = this.getNetworkResources(env, context.network);

    const ec2Service: ecs.Ec2Service = this.createECS(
      tag,
      context.container,
      vpc,
      securityGroup
    ...
    const albSecurityGroup: ec2.SecurityGroup = this.createSecurityGroup(
      context.network,
      vpc
    );

    const alb: elbv2.ApplicationLoadBalancer = this.createAlb(
      context.loadBalancer.alb,
      albSecurityGroup,
      vpc
    );
    ...
    this.addListener(
      alb,
      context.loadBalancer.listener,
      context.loadBalancer.targetGroup,
      context.container,
      ec2Service
    );

    this.createARecordWithAlb(context.route53, alb);
  }
